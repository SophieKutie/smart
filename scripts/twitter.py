...
class MyStreamListener(StreamListener):
    ...
    def on_limit(self, track):
        ...
        time.sleep(sleepy)
        self.nightnight += 1
        sys.stderr.write(f"on_limit {track} \n")
        return True
...
def run_script(term_list, time_limit=360, output_file='./test/data/tweets.json', reps=1000):
    for i in range(1, reps):
        print('Number of lists of terms to process: %d\n' % len(term_list))
        print(i, f' out of  {reps}  repitions.\n')
        k = 0
...
def main():
    ...
    terms_file = os.listdir('./media')  # '../media')
